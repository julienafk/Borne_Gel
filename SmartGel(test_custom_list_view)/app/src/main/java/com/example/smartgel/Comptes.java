...
//https://api.npoint.io/abb268c49134d4ea46b0 https://www.npoint.io/   http://51.210.151.13/btssnir/projets2022/bornegel/api/api/bornes.php
            try {
                ...
                if (!data.isEmpty()){
                    ...
                    int[] imageId = {R.drawable.compte,R.drawable.compte,R.drawable.compte,R.drawable.d,R.drawable.b,
                    ...
                    for(int i = 0;i< imageId.length;i++){
                        for (int y = 0;y<comptes.length();y++) {
                            JSONObject compte = comptes.getJSONObject(y);

                            User user = new User(compte.getString("prenom"),compte.getString("nom"),compte.getString("id"),compte.getString("email"),compte.getString("id"),imageId[i]);
                            userArrayList.add(user);
                        }
                    }

                    ListAdapter listAda = new ListAdapter(Comptes.this,userArrayList);

                    binding.comptesList.setAdapter(listAda);
                    binding.comptesList.setClickable(true);
//                    binding.comptesList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
//                        @Override
//                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
//
//                            Intent i = new Intent(Comptes.this,UserActivity.class);
//                            i.putExtra("nom", userArrayList.get(position)["nom"]);
